...
- `players_m` and `players_f` are data frames containing the **latest**
  ...
  - `"classement"`: classement at start of season (or after mid‑season
    reevaluation)
  ...
  - `"classement_chiffre"`: classement number (0, 2, 4, 6; 99 for A and
    NC)
...
- `players_m_archive` and `players_f_archive` are **named lists** of
  past snapshots. Each element is named `"YYYY_MM"` and contains a data
  frame with the same structure and attributes as the corresponding
  latest snapshot.
